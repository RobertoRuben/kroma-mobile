...
Java_com_ultralytics_yolo_ObjectDetector_postprocess(
    ...
    // Check if output is pre-processed format [N, 6] where N=detections, 6=[x1,y1,x2,y2,conf,class]
    // vs raw format [channels, anchors] where channels = 4 + num_classes
    bool isPreProcessedFormat = (w == 6 && h > 6);
    
    if (isPreProcessedFormat) {
        // Pre-processed format (NMS already applied by model): 
        // Each row is a detection [x1, y1, x2, y2, confidence, class_index]
        // where x1,y1 = top-left corner, x2,y2 = bottom-right corner (normalized 0-1)
        // h = number of detections, w = 6
        for (int i = 0; i < h; ++i) {
            float conf = vec[i][4];
            
            // Skip detections below confidence threshold or invalid confidence (>1.0 is impossible)
            if (conf <= confidence_threshold || conf > 1.0f) {
                continue;
            }
            
            // Format is [x1, y1, x2, y2, conf, class] - corner coordinates
            float x1 = vec[i][0];
            float y1 = vec[i][1];
            float x2 = vec[i][2];
            float y2 = vec[i][3];
            int class_index = static_cast<int>(vec[i][5]);
            
            // Calculate width and height from corners
            float w_box = x2 - x1;
            float h_box = y2 - y1;
            
            // Validate box dimensions - skip boxes that cover entire image or are invalid
            if (w_box >= 0.99f && h_box >= 0.99f) {
                continue; // Skip full-image boxes (usually invalid)
            }
            if (w_box <= 0.0f || h_box <= 0.0f) {
                continue; // Skip zero or negative size boxes
            }
            if (x1 < 0.0f || y1 < 0.0f || x2 > 1.0f || y2 > 1.0f) {
                continue; // Skip boxes outside valid range
            }
            
            DetectedObject obj;
            // x1, y1 are already top-left coordinates
            obj.rect.x = x1;
            obj.rect.y = y1;
        ...
        // Raw format: [channels, anchors] - original processing logic
        // w = number of anchors, h = channels (4 + num_classes)
        for (int i = 0; i < w; ++i) {
            int class_index = 0;
